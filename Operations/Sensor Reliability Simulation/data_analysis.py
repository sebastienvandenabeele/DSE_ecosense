...
if __name__ == "__main__":
    ...
    if plot:
        ...
        n,bins = np.histogram(np.deg2rad(data["wind_dir"]),bins=bins,density=True)
        ax[0,1].axis("off")
        ...
        corr_matrix = data[["max_temp","RH","wind_spd","wind_dir_dummy"]].corr()
        ...
        corr_heatmap.set_xticklabels(corr_heatmap.get_xticklabels(), rotation=0, horizontalalignment='center')
        ...
        bins = np.linspace(0,2*np.pi,18)
        n,bins = np.histogram(np.deg2rad(samples["wind_dir"]),bins=bins,density=True)
        ax[0,1].axis("off")
