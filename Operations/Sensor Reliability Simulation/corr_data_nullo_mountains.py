...
for col in fire_counts.columns.values:
    fire_stat[col] = (fire_counts[col]-fire_counts.mean(axis=1)
                      )/(fire_counts.std(axis=1))

fire_stat = fire_stat[fire_stat.abs() < 3]

for col in fire_counts.columns.values:
    fire_stat[col] = fire_stat[col] * \
        fire_counts.std(axis=1)+fire_counts.mean(axis=1)
