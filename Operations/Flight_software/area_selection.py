...
def select_area_map(topography,plot,dX,dY,dLat,dLong):
    ...
    if plot:
        ...
        radius = plt.Circle((park_perimeter.loc["Launch site","lat"],park_perimeter.loc["Launch site","long"]), 100*dX/dLat, color='b', fill=False,label="100km radius")
        fig,ax = plt.subplots(facecolor='#FCF6F5FF',sharex=True,sharey=True,figsize=(15,9))
        c = ax.imshow(topography, cmap=colormap)
        ax.plot(park_perimeter.loc["Park","lat"].values,park_perimeter.loc["Park","long"].values,linewidth=4,color = "tab:orange",label="park perimeter")
        ax.scatter(park_perimeter.loc["Launch site","lat"],park_perimeter.loc["Launch site","long"],s=200,label="Launch site",edgecolors = "k",zorder=5)
        ax.set_title("Activity map")
        ax.add_patch(radius)
        ax.axis("off")
        ax.set_ylim(park_perimeter.loc["Launch site","long"] + 1.5*100*dX/dLat, park_perimeter.loc["Launch site","long"] - 1.5*100*dX/dLat  )
        ax.set_xlim(park_perimeter.loc["Launch site","lat"] - 1.5*100*dX/dLat, park_perimeter.loc["Launch site","lat"] + 1.5*100*dX/dLat  )
        plt.legend()
